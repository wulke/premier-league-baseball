...
beforeEach(() => {
  ...
  global.fetch = jest.fn((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (/team\/10\/calendar/.test(url)) return response({ games: hasGame ? [{ gameId: 40, scheduledDate: '2025-04-05T00:00:00.000Z', homeTeamId: 10, homeTeamName: 'Mariners', awayTeamId: 11, awayTeamName: 'Rivertown', leagueId: 5, status: 'SCHEDULED' }] : [] });
    ...
    if (/game\/40\/simulate/.test(url) && method === 'POST') return response({ homeTeamResult: 5, awayTeamResult: 2 });
    return response({});
  }) as jest.Mock;
});
afterEach(cleanup);
