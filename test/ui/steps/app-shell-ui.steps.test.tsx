...
let world = { id: 1, year: 2025, currentDate: '2025-04-10', config: { name: 'Test World', inProgress: true }, Leagues: [{ id: 7, config: { name: 'Premier' } }] };
let calls: string[] = [];
...
beforeEach(() => {
    calls = [];
    world = { id: 1, year: 2025, currentDate: '2025-04-10', config: { name: 'Test World', inProgress: true }, Leagues: [{ id: 7, config: { name: 'Premier' } }] };
...
const registerSteps = ({ given, when, then }: any) => {
  ...
  given(/^GameWorld 1 is named "([^"]+)" with league 7 named "([^"]+)"$/, (name: string, league: string) => {
    world.config.name = name; world.Leagues[0].config.name = league;
  });
  ...
  then('the rail shows the app mark, HOME link, and disabled fog trio', () => { expect(screen.getByTestId('nav-mark')).toBeInTheDocument(); expect(screen.getByTestId('nav-home')).toBeInTheDocument(); expect(screen.getAllByTestId(/nav-fog-/)).toHaveLength(3); });
  then('the rail has no WORLD or COMPETITIONS section', () => { expect(screen.queryByTestId('nav-world')).toBeNull(); expect(screen.queryByTestId('nav-competitions')).toBeNull(); });
