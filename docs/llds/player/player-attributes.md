...
roster generation (`docs/llds/player/player-contracts-roster.md`, #64) or stats (`docs/llds/player/player-stats.md`,
...
| Field | Notes |
|---|---|
| `teamId` | Nullable FK — `null` represents a free agent. Not enforced non-null at the schema level. **Denormalized "current team" pointer**, not the authoritative record of the Player/Team relationship — `Contract` (`docs/llds/player/player-contracts-roster.md`) is. Kept alongside `Contract` for query ergonomics: "who is currently on Team X's roster" is the most common Player query and reading a plain FK is simpler than filtering `Contract` rows to the one covering the current `GameWorld.year`. See `docs/llds/player/player-contracts-roster.md`'s Edge Case Probe for the sync-responsibility trade-off this introduces. |
...
| **This LLD** | `docs/llds/player/player-attributes.md` |
| EARS | `docs/specs/player/player-attributes-specs.md` — `PATTR-001`.. |
