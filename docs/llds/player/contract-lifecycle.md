...
> Amends: `docs/llds/player/player-contracts-roster.md` (PCON-006/007 activation), `docs/llds/manager/roster-read-api.md` (ROST-004 closure), `docs/llds/player/player-detail-read-api.md` (no change — already anchors on `resolveCurrentContract`)
...
Transfers UI surface is `docs/llds/manager/transfers-ui.md` (sibling).
...
No session/user concept exists (`docs/llds/manager/managed-club-pointer.md`'s pointer is the only
...
so `GameWorldFactory.getFreeAgents()` (below, and `docs/llds/manager/transfers-ui.md`) can reuse it verbatim.

### Lineup repair (new — `src/db/domain/lineup.ts`)

`LineupFactory().generateActive()` (`docs/llds/manager/lineup-generation.md`) is idempotent — it returns the
