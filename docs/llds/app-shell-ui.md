...
For each render (driven by context gw + router):
  ...
  2. WORLD section renders only when gw != null:
       ...
       - current-date chip renders gw.currentDate (graduates SIMUI-006/007 — see EARS).            # SIMUI-006/007
...
| s4 | **NEW — the deferred current-date chip graduates into the rail** | `SIMUI-006`/`SIMUI-007` (chip display) were retired to `@future` by #44, explicitly "deferred to a future left-pane nav." That nav is this rail. They **graduate**: the WORLD section renders `gw.currentDate` formatted, and a muted placeholder when null. The two `@future` scenarios are re-tagged `@spec:SIMUI-006` / `@spec:SIMUI-007` and bound. | SIMUI-006/007 |
...
| Gherkin | `test/ui/features/app-shell-ui.feature` (NEW — shell scenarios) · `test/ui/features/simulate-game-ui.feature` (EDIT — re-homed batch/chip step text + `@future`→`@spec` for the chip) |
