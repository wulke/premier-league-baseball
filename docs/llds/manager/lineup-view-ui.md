...
| u13 | Wholesale save rejects or returns a malformed success payload | Keep the draft and edit mode, surface the server error or a generic save error, and make no read-card replacement. Only a canonical `TeamLineup` response replaces the read card and exits edit mode. | LINEUI-014 |
...
| u17 | A drop has no valid lineup-entry drag payload | Ignore the drop and leave the draft unchanged; only a drag initiated by this lineup's row may exchange slot occupants. | LINEUI-015 |
