...
1. `GameFactory(gameId)` loads `Game` by its primary key with read-only included
   PlayerGameStats/Player, Lineup/LineupEntry, and home/away Team associations; a missing
   game is `404`, and a non-completed game raises `DomainError(..., 422)`.
2. Join each `PlayerGameStats(gameId)` row to the matching frozen `LineupEntry` through
   `Lineup(gameId, teamId)` plus `playerId`.
...
- A stat row lacks a frozen entry -> omit it because it cannot be attributed safely; never
  fall back to mutable `Player.teamId`.
