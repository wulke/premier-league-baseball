...
const claimLeagueId = lowestNumericId(gw.Leagues ?? []);

// unclaimed → one prompt with a Link to `/${gwId}/${claimLeagueId}`
// claimed   → existing CalendarStrip + ActionItemsPanel composition
```

The GameWorld association read has no display-order contract, so the prompt routes to the
lowest-ID competition's existing team list. Selecting a team there opens
...
2. IF managedTeamId is null:
   ...
   b. Link to the lowest-ID competition so its existing team list leads to TeamHub.
...
|---|---|---|
| `managedTeamId` is `null` with one or more competitions | Show the single prompt and route deterministically to the lowest-ID competition's team list; array order is not relied upon. | UNCLMUI-001, UNCLMUI-002 |
