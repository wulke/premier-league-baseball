...
|---|---|---|
| LEDIT-001 | WHEN `PUT /api/team/:teamId/lineup` receives a complete valid roster-member entry set for the managed team THE system SHALL transactionally replace entries on the existing active Lineup row and return the canonical `TeamLineup` card. | [x] → #254 |
