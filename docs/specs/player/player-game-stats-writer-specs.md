...
|---|---|---|
| PGSW-001 | WHEN `GameFactory` completes a game through either single or batch simulation THE system SHALL obtain each participating team's frozen `Lineup(gameId)` through `TeamFactory(teamId).snapshotForGame(gameId)` and SHALL use its `LineupEntry` values, rather than the mutable active lineup, as player-stat attribution input | [ ] → #216 |
| PGSW-002 | WHEN a completing game's team has no lineup or an incomplete lineup including no valid starting pitcher THE system SHALL silently write no `PlayerGameStats` rows for that team's side and SHALL leave the game score completed | [ ] → #216 |
