...
| SIMUI-014 | WHEN POST /api/gameWorld/:gwId/simulate returns 200 with an in-progress game blocking date progression THE system SHALL show a warning indicating how many games could not be simulated that does NOT auto-dismiss and SHALL hide the "Simulate Today" button while the warning is active | [ ] |
