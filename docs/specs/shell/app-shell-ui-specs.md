...
| SHELL-004 | WHEN the NavRail renders THE system SHALL always render its branded app header as the active-aware home link (to "/") and SHALL NOT render a standalone HOME link; it SHALL also render the dimmed disabled fog trio (My Club, Roster, Transfers) | [ ] → #381 |
...
| SHELL-006 | WHEN the NavRail renders IF gw is not null THE system SHALL render a WORLD section whose HomeIcon-prefixed world name links to /:gwId | [ ] → #381 |
