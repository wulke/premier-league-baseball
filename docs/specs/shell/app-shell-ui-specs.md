...
| SHELL-004 | WHEN the NavRail renders THE system SHALL always render its branded app header as the active-aware home link (to "/") and SHALL NOT render a standalone HOME link; it SHALL also render the dimmed disabled fog trio (My Club, Roster, Transfers) | [x] → #381 |
...
| SHELL-006 | WHEN the NavRail renders IF gw is not null THE system SHALL render a WORLD section whose HomeIcon-prefixed world name links to /:gwId | [x] → #381 |
| SHELL-007 | WHEN the NavRail renders IF gw.Leagues is a non-empty array THE system SHALL render a COMPETITIONS section with one link per league to /:gwId/:leagueId, regardless of whether that league has an active season; IF gw.Leagues is empty or absent THE system SHALL render no competition links without error | [x] → #383 |
...
| SHELL-011 | WHEN content or navigation exceeds the viewport height THE App Shell SHALL keep the NavRail pinned at the viewport edge with an independent vertical scroll region, and SHALL make only the main content region scroll vertically | [x] → #294 |
