...
| LINEUI-011 | WHEN a starter is displayed in edit mode THE system SHALL derive its batting slot without allowing it to be edited; a newly assigned non-pitcher starter SHALL receive an available batting slot, and the pitcher SHALL be locked to 9 when DH is disabled and SHALL have no batting slot when DH is enabled. | [x] → #255 |
...
| LINEUI-014 | WHEN the manager saves the draft THE client SHALL PUT its complete assigned entry set to the wholesale save endpoint; on success it SHALL exit edit mode and use the returned card, and on 422 it SHALL retain the draft, remain in edit mode, and display the error. Cancel or route navigation SHALL discard the draft without a guard. | [x] → #255 |
