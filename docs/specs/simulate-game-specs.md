...
Backend requirements for single-game and batch game simulation (`GameFactory(id).simulate()` and `GameFactory().simulateBatch(gwId, endDate)` in `src/db/domain/game.ts`, exposed via `src/api/handlers.ts`).
