...
| XFER-021 | WHEN initial roster generation (`generateRoster()`) computes its starting Contract term THE system SHALL derive the term's end date from the same `SEASON_END_MONTH`/`SEASON_END_DAY` constants XFER-020 uses, rather than a separately hardcoded date | [x] → #237 |
...
ever Deferred. `XFER-013`/`XFER-017` supersede `PCON-007` (`docs/specs/player-contracts-specs.md`,
flipped from Deferred to Implemented by this map — see its Traceability note); `XFER-022` supersedes
`ROST-004` (`docs/specs/roster-read-api-specs.md`, likewise flipped). `PCON-006` is also flipped from
