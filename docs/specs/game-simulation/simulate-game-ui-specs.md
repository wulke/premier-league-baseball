...
| SIMUI-029 | WHEN a GameRow renders on the managed team's own calendar (`teamId === gw.managedTeamId`) THE system SHALL render a link alongside the existing "Simulate" button/result cell: `SCHEDULED` and `IN_PROGRESS` games SHALL use `/:gwId/:leagueId/game/:gameId`, while a `COMPLETED` game SHALL use `/:gwId/game/:gameId` so its box score route loader runs | [x] → #365, #389 |
