...
| PARP-014 | WHEN pitching stats are projected for a simulated game THE system SHALL accrue every out, hit, walk, strikeout, and earned run allowed by the batting team to that half-inning's single starting pitcher, SHALL increment `outsRecorded` rather than write `IP`, and SHALL add the `outsRecorded` column before accepting those projections on an existing SQLite database | [x] → #191 |
