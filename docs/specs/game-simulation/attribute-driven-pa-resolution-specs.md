...
|---|---|---|
| PARP-001 | WHEN the attribute-read seam reads a Player attribute for PA resolution THE system SHALL return the raw stored scalar as `iv` and `0` as `ev`, never a pre-combined effective value | [x] → #191 |
