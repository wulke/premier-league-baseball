...
|---|---|---|
| SCL-001 | WHEN a new `League` is created THE system SHALL initialize `League.year` to the parent `GameWorld.year` and `League.status` to `'CUTOVER'` | [ ] Active |
...
| SCL-012 | WHEN the `League.year`/`League.status` migration runs against an existing League THE system SHALL backfill `year = GameWorld.year` for every League, and `status = 'IN_SEASON'` if that League has any existing `DivisionSeason` row (any year), otherwise `status = 'CUTOVER'` | [ ] Active |
