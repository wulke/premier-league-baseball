...
| SCL-005 | WHEN `LeagueFactory(id).start()` succeeds THE system SHALL generate each Division's season (`DivisionSeason` + `Game` rows, existing round-robin/knockout/byes/two-leg logic unchanged) for `League.year`, and set `League.status` to `'IN_SEASON'` | [x] Implemented → #118 |
