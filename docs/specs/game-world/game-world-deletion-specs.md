...
| GWD-002 | WHEN `GameWorldFactory(id).delete()` is called IF the GameWorld exists THE system SHALL delete the GameWorld row together with every League, Team, Player, Division, DivisionSeason, Contract, PlayerGameStats, SeasonResult, and Notification it owns, and any Game (together with that Game's GameEvents) reachable only through its DivisionSeasons; Contract deletion SHALL be delegated to a `ContractFactory`-owned writer | [x] → #238, #373 |
