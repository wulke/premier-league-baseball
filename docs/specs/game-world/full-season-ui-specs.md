...
| UI-002 | WHEN both Leagues in a GameWorld are decided THE GameWorld hub SHALL show a computed `Season Complete` block naming both champions; WHILE only one is decided THE hub SHALL show that champion and `In progress` for the other | [D] — superseded by SHB-001 (#328) |
