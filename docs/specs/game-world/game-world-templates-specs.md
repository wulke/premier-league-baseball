...
|---|---|---|
| GWT-001 | WHEN a League template is promoted to a runnable game world THE system SHALL declare exactly one `GameWorldType` arm and one `DefaultWorlds` entry — kept exhaustive over `GameWorldType` so a missing entry fails to compile — with the template's team pool owned via `LeagueConfig.teams` (#283; pools moved off `NewGameWorld`) | [x] → #87, amended #283 |
