...
| TLO-010 | WHEN a GameWorld declares independent parent Leagues with distinct `matchRules` THE system SHALL generate each Team's initial active Lineup from its own Home League's resolved rules, regardless of `leagues[]` order; this SHALL NOT alter the separate Division-level override precedence (LIN-002) | [x] → #284 |
