...
| GBULL-007 | WHEN the managed team's next game is scheduled THE Bullpen tab SHALL let the manager drag an SP or active-bullpen slot onto another pitcher slot and a bench slot onto another bench slot, swap their draft occupants through the existing snapshot save flow, ignore cross-type drops, and expose no drag affordance when the game is locked or the team is not managed. | [x] → #251 |
