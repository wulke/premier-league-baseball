...
| ROSTUI-012 | WHEN the My Club Roster tab initially renders players THE system SHALL order them by primary defensive position `C, 1B, 2B, 3B, SS, LF, CF, RF, P`, then deterministically by player name within each position; IF the user selects a roster sort control THEN the system SHALL use that local sort order without refetching roster data | [x] → #307 |
