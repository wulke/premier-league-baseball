...
| PREGAME-005 | IF a managed club's scheduled game has a non-null `scheduledDate` that is after `GameWorld.currentDate`, or a non-null `scheduledDate` while `GameWorld.currentDate` is not configured, THE system SHALL render only the opponent-context section and SHALL NOT expose the lineup editor or "Ready to sim" control; a `scheduledDate` of `null` SHALL be treated as ready regardless of `GameWorld.currentDate`, matching the backend's `simulate()` guard. | [x] → #364 |
