...
| PREGAME-005 | IF a managed club's scheduled game has a `scheduledDate` after `GameWorld.currentDate`, or `GameWorld.currentDate` is not configured, THE system SHALL render only the opponent-context section and SHALL NOT expose the lineup editor or "Ready to sim" control. | [x] → #364 |
