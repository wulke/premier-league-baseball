...
| F-PGST-001 | Per-player box-score distribution | `src/db/domain/simulation/stat-projection.ts`; persistence in `src/db/domain/player-game-stats-writer.ts` | Attribute-driven simulation event chain; frozen `Lineup`/`LineupEntry` batting order, fielding position, starting pitcher; persisted player attributes | One `PlayerGameStats` row per participating starter; batting `SUM(R)` exactly reconciles to each team's engine score | Attribute-driven PA resolution (#191); fallback #190 `SimulationEngine` score result for unusable authored lineups | Player game/season/career stat reads; GameFactory single and batch simulation | A `GameFactory` completion with valid authored lineups succeeds; projection writes are inside the batch transaction | Implemented |
| F-PARP-001 | PA current-form IV/EV input | `src/db/domain/simulation/pa-resolver.ts` | Per-PA `ivEv` attribute tuple (`IV`, earned `EV`) for discipline, accuracy, contact, arm strength, and power | The five current-game PA inputs, each `0.3 × IV + 0.9 × EV`, feeding the existing weighted outcome distribution | F-IVEV-001's tuple contract; PA resolver weight normalization | `AttributeDrivenSimulationEngine`; live authored-lineup simulation | Each plate appearance before its one RNG outcome draw | Implemented |
