...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F-IVEV-001 | Effective person-attribute read | Future IV/EV pattern module | Person attribute state (`IV`, `EV`, `Nature`, `formWindow`, `ageDiscountMeta`), per-attribute aging profile, consumer read policy, read time | A consumer's faded-capacity/form/aggregate input; normally a clamped consumed value | F-IVEV-002 | Future game, manager, scout, and salary formula catalog entries | A consumer requests an attribute read | Draft |
