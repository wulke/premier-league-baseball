...
| F-IVEV-004 | Applicable-Nature resolution | Future IV/EV pattern module | Fixed person-level named Natures, attribute kind, Nature catalog | Fixed expression multiplier for that attribute; `1` when no Nature applies | None | F-IVEV-001 | F-IVEV-001 reads an attribute | Draft |
