...
// @spec PREGAME-001,PREGAME-002,PREGAME-003,PREGAME-004,BADGEUI-010 — game context is loaded from the
...
const PreGamePrep = () => {
  ...
  return <PageContainer as="main" style={{ padding: '24px 24px 48px' }}>
    {/* @spec TEAMLINK-005 */}
    <header style={{ marginBottom: '20px' }}><SectionLabel>Pre-game prep</SectionLabel><h1 style={{ margin: '4px 0', display: 'flex', alignItems: 'center', gap: '8px' }}><TeamCrest name={game.homeTeamName} badge={game.homeTeamBadge} size={24} testId={`pre-game-team-badge-${game.homeTeamId}`} /><TeamLink gwId={gwId} teamId={game.homeTeamId}>{game.homeTeamName}</TeamLink> vs <TeamCrest name={game.awayTeamName} badge={game.awayTeamBadge} size={24} testId={`pre-game-team-badge-${game.awayTeamId}`} /><TeamLink gwId={gwId} teamId={game.awayTeamId}>{game.awayTeamName}</TeamLink></h1><p style={{ margin: 0, color: '#666' }}>{game.scheduledDate ? (formatUtcDate(game.scheduledDate) ?? 'Date TBD') : 'Date TBD'} · {game.homeTeamId === managedTeamId ? 'Home' : 'Away'}</p></header>
    <Card as="section" aria-label="Opponent context" style={{ padding: '16px', marginBottom: '20px' }}><SectionLabel>Opponent</SectionLabel><strong style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}><TeamCrest name={opponentName} badge={opponentBadge} size={22} testId={`pre-game-opponent-badge-${opponentId}`} /><TeamLink gwId={gwId} teamId={opponentId}>{opponentName}</TeamLink></strong><div style={{ marginTop: '8px', color: '#555' }}>Record: {record ? `${record.won}-${record.lost}${record.drawn ? `-${record.drawn}` : ''}` : '—'}</div><div style={{ marginTop: '4px', color: '#555' }}>Probable pitcher: {probablePitcher ? `${probablePitcher.givenName} ${probablePitcher.familyName}` : 'TBD'}</div></Card>
