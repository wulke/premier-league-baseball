...
const Stats = ({ playerId, gwId }: { playerId: string | undefined; gwId: string | undefined }) => {
  const [grain, setGrain] = useState<'season' | 'career'>('season'); const [stats, setStats] = useState<any>();
  useEffect(() => { setStats(undefined); fetch(`/api/player/${playerId}/stats?gwId=${gwId}&grain=${grain}`).then((r) => r.ok ? r.json() : null).then(setStats); }, [playerId, gwId, grain]);
  return <div data-testid="player-stats"><div role="group" aria-label="Stats grain">{(['season', 'career'] as const).map((key) => <button key={key} type="button" aria-pressed={grain === key} onClick={() => setGrain(key)}>{key === 'season' ? 'Season' : 'Career'}</button>)}</div>{stats === undefined ? <p>Loading stats…</p> : stats == null ? <p data-testid="player-stats-empty">No stats recorded yet</p> : <>{(['batting', 'pitching'] as const).map((kind) => <Panel key={kind}><h2>{kind === 'batting' ? 'Batting' : 'Pitching'}</h2><dl data-testid={`${kind}-stats`}>{Object.entries(stats[kind] as Record<string, React.ReactNode>).map(([key, value]) => <React.Fragment key={key}><dt>{key}</dt><dd>{value ?? '—'}</dd></React.Fragment>)}</dl></Panel>)}</>}</div>;
};
