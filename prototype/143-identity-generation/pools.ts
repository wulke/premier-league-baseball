 ...
 *
 * Structure: a REGISTRY of countries — { code, display, flag, given[], family[] }
 * — holding only global, league-agnostic data (the name pools). Nationality
 * WEIGHTS are NOT baked in here; they live in per-league compositions (see
 * compositions.ts), so the same pools serve an MLB-style league, KBO, NPB, or any
 * future global league.
 *
 * Pick countryCode FIRST (weighted by the active league's composition), then draw
 * given/family from that country's pool → name & country consistent by construction.
 *
 * The "long tail" (BR/PA/CO/NI…, plus European countries for a Euro league) is
 * intentionally omitted for v1 — adding countries is just adding registry entries
 * + pools (no schema cost). It's a tunable on the ticket.
 *
